...
def generate_screenshots():
    ...
    for i in range(4):
        ...
        draw.text((250, 60), "Real-time weld seam anomaly screening and visual audit log generator.", fill=(150, 150, 150))
        draw.rectangle([250, 90, 780, 92], fill=(80, 80, 80))
        
        if i == 0:
            # Frame 1: Awaiting Upload
            draw.text((250, 120), "Step 1: Upload a weld seam image or use default sample", fill=text_color)
            draw_rounded_rect(draw, [250, 160, 750, 320], 10, card_bg)
            draw.text((420, 230), "📁 Drag and drop weld image here", fill=(150, 150, 150))
            ...
            draw.text((250, 120), "Step 1: Upload a weld seam image or use default sample", fill=(150, 150, 150))
            draw_rounded_rect(draw, [250, 160, 750, 320], 10, card_bg)
            draw.text((360, 220), "⌛ Running Late-Fusion Coreset Optimization...", fill=accent_color)
            ...
            draw.text((250, 110), "Quality Inspection Complete! 🔴 Anomaly Detected", fill=(255, 75, 75))
            ...
            draw.text((320, 180), "★ IATF 16949 QUALITY ASSURANCE AUDIT RECORD ★", fill=(255, 215, 0))
            draw.text((280, 220), "Chassis VIN:  BMW-G60-2026", fill=text_color)
            draw.text((280, 255), "Timestamp:    2026-05-31 00:06:12 UTC", fill=text_color)
            draw.text((280, 290), "Inspection:  FAIL  |  Core Score: 0.8966  |  Limit: 0.50", fill=text_color)
            draw.text((280, 325), "Certified Secure Visual Audit Hash: sha256_e10dced...", fill=(120, 120, 120))
            
            # Download Button
            draw_rounded_rect(draw, [250, 410, 750, 460], 5, (0, 200, 100))
            draw.text((430, 425), "📥 Download Certified Quality PDF Report", fill=text_color)
