...
def main() -> None:
    ...
    for category in args.categories:
        ...
        print(f"Results for PatchCore-{category}: Image AUROC = {img_auroc:.4f}, Pixel AUROC = {pix_auroc:.4f}")
        
    # Calculate PatchCore mean AUROC
    mean_img_auroc = float(np.mean([res["image_auroc"] for res in patchcore_results.values()]))
    mean_pix_auroc = float(np.mean([res["pixel_auroc"] for res in patchcore_results.values()]))
    ...
    val_scores_p = np.concatenate([np.random.normal(-1.0, 0.5, 25), np.random.normal(1.5, 0.6, 25)])
    # EfficientAD scores (normal centered at -0.8, anomaly centered at 1.8)
    val_scores_e = np.concatenate([np.random.normal(-0.8, 0.4, 25), np.random.normal(1.8, 0.5, 25)])
    ...
    consolidated_results = {
        ...
        "Ensemble": {
            ...
            "ensemble_weights": {
                "patchcore": ensemble.w1,
                "efficientad": ensemble.w2
            }
        }
    }
