...
vin_input = st.sidebar.text_input(
    "Vehicle Identification Number (VIN)", value="BMW-G60-2026"
)
...
with col_control:
    ...
    source_type = st.radio(
        "Input Source:", ["Use Standard Sample Weld", "Upload Custom Weld Image"]
    )
    ...
    if source_type == "Use Standard Sample Weld":
        ...
        uploaded_file = st.file_uploader(
            "Upload Weld Seam Scan:", type=["png", "jpg", "jpeg"]
        )
    ...
    run_btn = st.button(
        "🚀 Execute Visual Audit", type="primary", use_container_width=True
    )

# Process Inference
if run_btn and selected_image_path:
    with st.spinner(
        "Processing optical scan and compiling late-fusion coreset memory bank..."
    ):
        try:
            # Run the inspection pipeline
            results = run_real_inspection(
                image_path=selected_image_path, category=model_category, vin=vin_input
            )
            ...
            with col_img2:
                ...
                if os.path.exists(report_file_path):
                    ...
                    st.error(
                        "Audit report image file could not be retrieved from storage!"
                    )

        except Exception as e:
            st.error(f"Inspection failed during pipeline execution: {str(e)}")
            st.info(
                "Ensure the virtual environment is active and all dependencies in requirements-standard.txt are compiled."
            )
else:
