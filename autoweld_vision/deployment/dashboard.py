...
menu = st.sidebar.radio(
    "Navigation",
    ["Live Monitor", "Quality Analytics", "Audit Explorer", "Inference Lab"],
)
    ...
    try:
        ...
        if isinstance(image_path_or_bytes, str):
            with open(image_path_or_bytes, "rb") as f:
                files = {
                    "file": (os.path.basename(image_path_or_bytes), f, "image/png")
                }
                response = requests.post(
                    f"{API_BASE_URL}/inspect?vin={vin}", files=files, timeout=2.0
                )
        else:
            image_bytes = image_path_or_bytes.getvalue()
            files = {
                "file": ("uploaded_image.png", io.BytesIO(image_bytes), "image/png")
            }
            response = requests.post(
                f"{API_BASE_URL}/inspect?vin={vin}", files=files, timeout=2.0
            )
...
if menu == "Live Monitor":
    ...
    with col_left:
        ...
        if os.path.exists("test_weld.png"):
            st.image(
                "test_weld.png",
                caption="Live Camera Feed - Station B4",
    ...
    if files:
        ...
        if selected_file:
            st.image(
                os.path.join(AUDIT_LOGS_DIR, selected_file), use_container_width=True
            )
            with open(os.path.join(AUDIT_LOGS_DIR, selected_file), "rb") as file:
                st.download_button(
                    "Download Official Audit PNG", file, file_name=selected_file
                )
    else:
    ...
    uploaded_file = st.file_uploader(
        "Choose a weld image...", type=["jpg", "png", "jpeg"]
    )
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 Run SOTA Inspection"):
            with st.spinner("Analyzing with Anomaly Engine..."):
                ...
                if (
                    "audit_report" in res
                    and res["audit_report"]
                    and os.path.exists(res["audit_report"])
                ):
                    st.image(
                        res["audit_report"],
                        caption="Generated IATF 16949 Audit Report",
                        use_container_width=True,
                    )
