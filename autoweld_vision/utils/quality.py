...
class IndustrialQualityAuditor:
    ...
    def generate_anomaly_heatmap(self, image: np.ndarray, anomaly_map: np.ndarray) -> np.ndarray:
        """Overlays a jet-color heatmap on the original image."""
        # Normalize anomaly map
        amap = (anomaly_map - anomaly_map.min()) / (anomaly_map.max() - anomaly_map.min() + 1e-8)
    ...
    def save_audit_report(self, vin: str, image: np.ndarray, anomaly_map: np.ndarray, 
        ...
        plt.suptitle(f"Quality Decision: {decision} | IATF 16949 Audit Trail", fontsize=16)
        plt.tight_layout()
