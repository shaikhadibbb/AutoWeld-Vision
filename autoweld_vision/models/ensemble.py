...
class AnomalyEnsemble(BaseAnomalyModel):
    ...
    def __init__(self, models: Dict[str, nn.Module]) -> None:
        ...
        self.w1: float = 0.5
        self.w2: float = 0.5
        self._learned_weights: Dict[str, float] = {name: 1.0 / len(models) for name in models.keys()}
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, Any]:
        ...
        scores: Dict[str, torch.Tensor] = {name: res["score"] for name, res in results.items()}
    ...
    def optimize_weights(self, val_scores_m1: np.ndarray, val_scores_m2: np.ndarray, val_labels: np.ndarray) -> None:
        """
        ...
        res = minimize(bce_loss, initial_weights, method='SLSQP', bounds=bounds, constraints=cons)
        ...
        if res.success:
            ...
            print(f"✓ Ensemble weights optimized! w1 ({model_names[0]}): {self.w1:.4f}, w2 ({model_names[1]}): {self.w2:.4f}")
        else:
