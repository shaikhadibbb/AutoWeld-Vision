...
class DomainShiftEvaluator:
    ...
    def evaluate_cross_dataset(self, source_name: str, target_dataloader: torch.utils.data.DataLoader):
        ...
        for name, model in self.models.items():
            ...
            report = AdvancedIndustrialMetrics.generate_full_report(np.array(labels), np.array(scores))
            results[name] = report
            ...
            for batch in dataloader:
                ...
                for i in range(len(labels)):
                    if preds[i] == 1 and labels[i] == 0:
                        fps.append({'image': images[i], 'score': scores[i].item(), 'path': paths[i]})
                    elif preds[i] == 0 and labels[i] == 1:
                        fns.append({'image': images[i], 'score': scores[i].item(), 'path': paths[i]})
...
if __name__ == "__main__":
    ...
    loader = MVTecADLoader(root='./datasets/mvtec', category='bottle', split='test', transform=transform)
