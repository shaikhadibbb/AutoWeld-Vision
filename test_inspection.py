...
def run_real_inspection(image_path: str, category: str = "bottle", vin: str = "UNKNOWN") -> Dict[str, Any]:
    ...
    resized_amap = Image.fromarray(anomaly_map).resize((orig_w, orig_h), Image.Resampling.BILINEAR)
    ...
    fig.suptitle(f"{status_text}\nIATF 16949 Audit Trail | Model: PatchCore v1.0.0 | Time: {timestamp}", fontsize=12)
...
def main() -> None:
    ...
    parser.add_argument("--vin", default="UNKNOWN", help="Vehicle Identification Number")
    parser.add_argument("--category", default="bottle", help="MVTec category to inspect against")
